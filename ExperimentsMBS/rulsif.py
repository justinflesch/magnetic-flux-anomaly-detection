...
# TODO Unify these two first functions

def TimeSeriesDissimilarityStepped(dataset, StepSize, SampleWidth, RetroWidth, Sigma, Alpha, Lambda):
    # SampleWidth is k in the paper, seen on page 4
    # RetroWidth is n in the paper, seen on page 4
    
    # Footnote seems to suggest flattening the dataset?
    ...
    dissimilarities = np.zeros(((len(Subsequences) - 2*RetroWidth - 1) // (StepSize) + 1, 3, 2))
    for resIndex, baseIndex in enumerate(range(0, len(Subsequences) - 2*RetroWidth, StepSize)):
...
def RelativeDivergence(Y1, Y2, Thetas, Sigma, Alpha):
    ...
    # Should vectorize usage of EstimateRatio
